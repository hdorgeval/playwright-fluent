...
describe.skip('uncheck handle', (): void => {
  ...
  test('should throw when selector is disabled and checked - chrome', async (): Promise<void> => {
    // Given
    browser = await chromium.launch({ headless: true, executablePath: getChromeCanaryPath() });
    const browserContext = await browser.newContext({ viewport: null });
    const page = await browserContext.newPage();
  ...
  test('should do nothing when selector is already unchecked - chrome', async (): Promise<void> => {
    // Given
    browser = await chromium.launch({ headless: true, executablePath: getChromeCanaryPath() });
    const browserContext = await browser.newContext({ viewport: null });
    const page = await browserContext.newPage();

...
describe('do-not-get-handle-of', (): void => {
  ...
  test('should return null when selector does not exist on the page - chromium', async (): Promise<void> => {
    // Given
  ...
  test('should throw an error when selector does exist on the page - chromium', async (): Promise<void> => {
    // Given
    browser = await chromium.launch({ headless: true });
    const context = await browser.newContext({ viewport: null });
    const page = await context.newPage();

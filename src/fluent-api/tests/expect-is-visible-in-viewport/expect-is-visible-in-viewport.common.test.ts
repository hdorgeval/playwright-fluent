...
describe('Playwright Fluent - expect is visible in viewport', (): void => {
  ...
  test('should give back an error on expectThat(selector-fluent).isVisibleInViewport when browser has not been launched', async (): Promise<void> => {
    // Given
    ...
    let result: Error | undefined = undefined;
    try {
      await p.expectThatSelector(selector).isVisibleInViewport();
    } catch (error) {
      result = error as Error;
    }
